...
class SerperService:
    ...
    def raw_search(self, query: str, num_results: int = 10) -> list:
        """
        Perform a raw Google search and return a list of simplified result dicts.
        Each item has: title, snippet, link.
        Returns empty list on error.
        """
        raw = self.search(query, num_results=num_results)
        if not raw:
            return []
        results = []
        ...
        # Also include knowledge graph description if present
    ...
    # Targets the /places endpoint to retrieve real-world business category,
    # website, and phone number for local service business classification.
    # =========================================================================
    ...
    def search_google_places(
        ...
        """
        Search for a business on Google Maps via the Serper Places API.

        Formats the query as ``"{business_name}, {city}, {state}"`` and returns
        the top matching place's key fields.

        Args:
            business_name: Name of the business (e.g. "Joe's Plumbing")
            city:          City where the business operates (e.g. "Miami")
            state:         Two-letter state code or full name (e.g. "FL")

        Returns:
            A dict with the following keys (all may be None if not found):
              - ``category``    : Google Maps category (e.g. "Plumber")
              - ``website``     : Business website URL
              - ``phoneNumber`` : Business phone number
              - ``address``     : Full address string from Google Maps
              - ``rating``      : Float rating (e.g. 4.5)
              - ``place_name``  : Name as it appears on Google Maps
              - ``raw_place``   : The full raw place dict from the API response
        """
        empty_result = {
            "category": None,
            "website": None,
        ...
        if not self.is_configured():
            logger.warning("Serper API key not configured - cannot call Places endpoint")
            return empty_result

        # Build the query exactly as the prompt specifies
        query = f"{business_name}, {city}, {state}"
        logger.info(f"[Places] Querying Serper Places: '{query}'")
        ...
        payload = {
            "q": query,
            "num": 5,  # Retrieve a few candidates in case the top hit is wrong
        }
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"[Places] Serper Places API error: {response.status_code} - {response.text[:200]}"
                )
                return empty_result
            ...
            if not places:
                logger.info(f"[Places] No places found for query: '{query}'")
                return empty_result

            # Pick the best match: prefer a place whose title closely matches
            # the business name we searched for.
            ...
            for place in places:
                ...
                title_words = set(place_title.split())
                overlap = len(name_words & title_words)
                if overlap > best_score:
                    best_score = overlap
                    best_place = place

            # Fall back to the first result if nothing matched well
            if best_place is None:
                best_place = places[0]

            category    = best_place.get("category") or best_place.get("type")
            website     = best_place.get("website")
            phone       = best_place.get("phoneNumber")
            address     = best_place.get("address")
            rating      = best_place.get("rating")
            place_name  = best_place.get("title")

            logger.info(
                f"[Places] Match for '{business_name}': "
                f"category='{category}', phone='{phone}', website='{website}'"
            )
            ...
            return {
                ...
                "raw_place": best_place,
            }

        except requests.RequestException as exc:
            logger.error(f"[Places] Request failed for '{query}': {exc}")
            return empty_result
        except Exception as exc:
            logger.error(f"[Places] Unexpected error for '{query}': {exc}")
            return empty_result
