...
class RealScraperBase(BaseScraper):
    """Base class for real state scrapers with anti-bot infrastructure."""
    ...
    def _make_request(self, url: str, method: str = 'GET', max_retries: int = 3, **kwargs) -> Optional[requests.Response]:
        """Make a rate-limited request with retry and CAPTCHA detection."""
        # Enforce rate limit
        ...
        try:
            return make_request_with_retry(
                self.session, url, method=method,
                max_retries=max_retries, base_delay=self.delay,
                **kwargs,
            )
...
class FloridaScraper(RealScraperBase):
    ...
    def fetch_new_businesses(self, limit: int = 50) -> List[BusinessRecord]:
        ...
        # Search generally across the alphabet and common entity suffixes to capture ALL businesses
        search_terms = ['LLC', 'INC'] + list(string.ascii_uppercase)
...
class SampleDataScraper(BaseScraper):
    ...
    def fetch_new_businesses(self, limit: int = 50) -> List[BusinessRecord]:
        ...
        for i in range(min(limit, 30)):
            ...
            # Recent filing dates
            days_ago = i % 14
            filing_date = (base_date - datetime.timedelta(days=days_ago)).strftime("%Y-%m-%d")
