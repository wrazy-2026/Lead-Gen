...
class Database:
    ...
    def _init_firebase(self):
        ...
        # Use (default) as the primary fallback since most Firestore setups only have one database.
        # Passing None to firestore.client() automatically chooses the default/first database.
        firestore_db_id = os.environ.get('FIRESTORE_DATABASE_ID')
        if firestore_db_id:
            firestore_db_id = firestore_db_id.strip()
        
        try:
            # Use specific database if set, else default to None (automatic)
            self.db = firestore.client(database_id=firestore_db_id if firestore_db_id else None)
            logger.info(f"Firestore client initialized (Project: {self.db.project}, Database: {firestore_db_id or '(default)'})")
            self.leads_ref = self.db.collection('leads')
        except Exception as e:
            logger.error(f"Failed to initialize Firestore with DB ID '{firestore_db_id}': {e}")
            # Emergency fallback: try default client
            self.db = firestore.client()
            self.leads_ref = self.db.collection('leads')
            logger.info("Connected to default Firestore fallback.")
    ...
    def save_records(self, records: List[BusinessRecord]) -> Tuple[int, int, List[str]]:
        ...
        for doc_id, record_dict in to_process:
            ...
            fetched_at = record_dict.get("fetched_at") or datetime.datetime.now().isoformat()
