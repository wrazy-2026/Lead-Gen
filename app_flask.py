...
initialize_firebase() # CRITICAL: Must be called before other imports that use Firestore

import threading
import traceback # Added by user instruction
import pandas as pd
...
def scraper_dashboard():
    """Scraper management and status dashboard."""
    from scraper_manager import get_manager
    manager = get_manager()
    stats = manager.get_last_run_stats()
    scrapers = manager.list_available_scrapers()
    
    # States that were previously missing but now covered by SEC Global
    missing_states_covered = [
        'AL', 'AK', 'AZ', 'AR', 'CO', 'CT', 'GA', 'HI', 'ID', 'IL', 
    ...
    return render_template('scraper_dashboard.html', 
                          stats=stats, 
                          scrapers=scrapers,
                          missing_states=missing_states_covered)
...
def do_fetch():
    ...
    try:
        ...
        for s in states:
            state_results[s] = {'count': 0, 'records': []}
        ...
        if missing_states and use_sec:
            ...
            def fetch_state_sec(state_code):
                try:
                    sec_scraper = SECEdgarScraper()
                    # Search specifically for this state on SEC
                    recs = sec_scraper.fetch_new_businesses(limit=limit, state_code=state_code, fast_mode=False)
                    return state_code, recs, None
                except Exception as ex:
                    return state_code, [], str(ex)

            with ThreadPoolExecutor(max_workers=5) as executor:
                future_to_state = {executor.submit(fetch_state_sec, s): s for s in missing_states}
                for future in as_completed(future_to_state):
                    s_code, recs, err = future.result()
                    if recs:
                        if s_code not in state_results:
                             state_results[s_code] = {'count': 0, 'records': []}
                        state_results[s_code]['count'] += len(recs)
                        state_results[s_code]['records'].extend(recs)
                        all_records.extend(recs)
                        scraper_results.append(f"{s_code}: Found {len(recs)} leads via Targeted SEC State Search")
                    elif err:
                        # Don't add to scraper_results yet, wait for fallback
                        pass
                    else:
        ...
        def _classify_category(name, state=None, address=None):
            """Use Gemini when available, then fallback to keyword-based category detection."""
            if gemini_svc and getattr(gemini_svc, 'model', None):
                try:
                    ai_category = gemini_svc.classify_business_category(
                        business_name=name,
                        state=state,
                        address=address
                    )
        ...
        if missing_states:
            ...
            def fetch_fallback(state_code):
                try:
                    state_scraper = get_real_scraper(state_code)
                    if state_scraper and state_scraper.is_available():
                        recs = state_scraper.fetch_new_businesses(limit=limit)
                        # Detect business category
                        for r in recs:
                            if not r.industry_category:
                                r.industry_category = _classify_category(
                                    r.business_name,
                                    getattr(r, 'state', None),
                                    getattr(r, 'address', None) or getattr(r, 'business_address', None)
                                )
                        return state_code, recs, None
                    return state_code, [], "Not available"
                except Exception as ex:
                    return state_code, [], str(ex)

            # Parallel fallback execution
            with ThreadPoolExecutor(max_workers=5) as executor:
                future_to_state = {executor.submit(fetch_fallback, s): s for s in missing_states}
                for future in as_completed(future_to_state):
                    s_code, recs, err = future.result()
                    if recs:
                        state_results[s_code] = {'count': len(recs), 'records': recs}
                        all_records.extend(recs)
                        scraper_results.append(f"{s_code}: {len(recs)} leads (Fallback)")
                    elif err:
                        scraper_results.append(f"{s_code}: {err}")
                    else:
                        scraper_results.append(f"{s_code}: No results found")

        # Apply AI category detection to any records still missing it
        for record in all_records:
            if not record.industry_category:
                record.industry_category = _classify_category(
                    record.business_name,
                    getattr(record, 'state', None),
                    getattr(record, 'address', None) or getattr(record, 'business_address', None)
                )
        ...
        def get_clean_key(record):
            """Generate a robust deduplication key."""
            # Basic Name + State
            import re
            name_clean = re.sub(r'[^a-z0-9]', '', record.business_name.lower())
            state = (record.state or "US").upper()
            
            # Add phone if available (very strong unique identifier)
            phone = re.sub(r'\D', '', record.phone or record.business_phone or '')
            if phone and len(phone) >= 10:
                return f"{name_clean}_{state}_{phone}"
            
            # Add address if available (strong, but handle generic agent addresses)
            addr = re.sub(r'[^a-z0-9]', '', (record.business_address or record.address or '').lower())
            if addr and len(addr) > 10:
                # To handle cases like "1209 North Orange St" (Delaware agents), 
                # we still include the name
                return f"{name_clean}_{state}_{addr[:30]}"
        ...
        for record in all_records:
            key = get_clean_key(record)
            if key not in unique_records:
                unique_records[key] = record
            else:
                # If existing record has no phone/address but this one does, prefer this one
                existing = unique_records[key]
                if not (existing.phone or existing.business_phone) and (record.phone or record.business_phone):
                    unique_records[key] = record
        ...
        session['last_state_results'] = state_results
        session['last_total_states'] = len(states)
        session['last_total_leads'] = saved
        ...
        if saved > 0 and (use_serper or use_apify):
            ...
            def run_background_enrichment():
                try:
                    ...
                    # ----------------------------------------------------------------
                    # STEP 2: OWNER LOOKUP
                    # Use Serper to search "[business name] owner" and Gemini to extract
                    # first/last name from the raw search results bundle.
                    # ----------------------------------------------------------------
                    if use_serper:
                        ...
                        if serper.is_configured():
                            for lead in leads_to_enrich:
                                ...
                                try:
                                    ...
                                    biz_state = lead.get('state', '')
                                    biz_phone = lead.get('business_phone', '') or lead.get('phone', '')
                                    biz_address = lead.get('address', '') or lead.get('business_address', '')
                                    biz_domain = lead.get('website', '') or lead.get('serper_domain', '')

                                    # 2A. Serper: search for owner name via web search
                                    search_query = f'"{biz_name}" {biz_state} owner OR founder OR CEO'
                                    raw_results = serper.raw_search(search_query)

                                    # 2B. Gemini: analyse results to extract first/last name
                                    first_name, last_name, owner_name = None, None, None
                                    if raw_results and gemini_svc:
                                        try:
                                            prompt = (
                                                f"You are a data extraction assistant. Below are web search results for a business called "
                                                f"'{biz_name}' located in {biz_state}.\n\n"
                                                f"Search Results:\n{str(raw_results)[:3000]}\n\n"
                                                f"Task: Extract the OWNER or FOUNDER's first name and last name from these results. "
                                                f"Look for titles like Owner, Founder, CEO, President, Managing Member, or Principal. "
                                                f"Return ONLY a JSON object in this exact format with no extra text:\n"
                                                f'{{ "first_name": "...", "last_name": "...", "confidence": 0.0 }}\n'
                                                f"If you cannot find a real name, return: "
                                                f'{{ "first_name": null, "last_name": null, "confidence": 0.0 }}'
                                            )
                                            import json as _json
                                            gemini_response = gemini_svc.generate_text(prompt)
                                            # Parse Gemini JSON response
                                            resp_text = gemini_response.strip()
                                            # Extract JSON block if wrapped in markdown
                                            if '```' in resp_text:
                                                resp_text = resp_text.split('```')[1]
                                                if resp_text.startswith('json'):
                                                    resp_text = resp_text[4:]
                                            parsed = _json.loads(resp_text.strip())
                                            fn = parsed.get('first_name')
                                            ln = parsed.get('last_name')
                                            conf = float(parsed.get('confidence', 0))
                                            if fn and ln and conf >= 0.5:
                                                first_name = fn.strip()
                                                last_name = ln.strip()
                                                owner_name = f"{first_name} {last_name}"
                                        except Exception as ge:
                                            print(f"[Gemini] Owner name extraction error: {ge}")
                                            # Fallback: use legacy serper search
                                            try:
                                                legacy_result = serper.search_business_owner(
                                                    biz_name, biz_state, biz_address, biz_phone
                                                )
                                                if legacy_result.owner_name and legacy_result.confidence >= 0.6:
                                                    owner_name = legacy_result.owner_name
                                    ...
                                    # 2C. Also run the full Serper lookup for domain/website
                                    update_data = {}
                                    try:
                                        full_result = serper.search_business_owner(
                                            biz_name, biz_state, biz_address, biz_phone
                                        )
                                        if full_result.website:
                                            update_data['website'] = full_result.website
                                            update_data['serper_domain'] = full_result.domain
                                        if full_result.business_category:
                                            update_data['industry_category'] = full_result.business_category
                                        # Only use Serper owner name if Gemini found nothing
                                        if not owner_name and full_result.owner_name and full_result.confidence >= 0.7:
                                            owner_name = full_result.owner_name
                                            parts = owner_name.split()
                                            first_name = parts[0] if parts else None
                                            last_name = ' '.join(parts[1:]) if len(parts) > 1 else None
                                    except:
                                    ...
                                    if owner_name:
                                        ...
                                        update_data['serper_owner_name'] = owner_name
                                    if first_name:
                                    ...
                                    if update_data:
                                        ...
                                        # Sync back to list for Apify step
                                        lead.update(update_data)
                                        task.enriched += 1

                                except Exception as e:
                                    task.failed += 1
                                    print(f"[Enrichment Step 2] Error for {lead.get('business_name')}: {e}")

                    # ----------------------------------------------------------------
                    # STEP 3: CONTACT ENRICHMENT via Apify
                    # Uses the now-known owner first/last name + business info
                    # to get complete contact details (emails, phones, socials, etc.)
                    # ----------------------------------------------------------------
                    if use_apify:
                        try:
                            ...
                            for i, enriched_biz in enumerate(enriched_results):
                                ...
                                # Only save if we got meaningful contact data
                                if any([
                                    enriched_biz.get('email'), enriched_biz.get('email_1'),
                                    enriched_biz.get('phone'), enriched_biz.get('phone_1')
                                ]):
                                    apify_update = {
                                        ...
                                        'dob': enriched_biz.get('dob') or enriched_biz.get('date_of_birth'),
                                        'age': enriched_biz.get('age'),
                                        'linkedin_url': enriched_biz.get('linkedin_url'),
                                        'enrichment_source': enriched_biz.get('enrichment_source', 'apify'),
                                        'confidence_score': enriched_biz.get('confidence_score', 0)
                                    }
                                    # Only overwrite owner name if Apify found something and Step 2 didn't
                                    if not leads_to_enrich[i].get('first_name') and enriched_biz.get('first_name'):
                                        apify_update['first_name'] = enriched_biz.get('first_name')
                                        apify_update['last_name'] = enriched_biz.get('last_name')
                                        apify_update['owner_name'] = enriched_biz.get('owner_name')
                                    db.update_lead_enrichment(lead_id, apify_update)
                        except Exception as e:
                            print(f"[Enrichment Step 3] Apify error: {e}")

                    task.status = 'completed'
                    task.completed_at = datetime.now()
                    invalidate_cache()
                    print(f"[Enrichment] Task {task_id} done: {task.enriched} enriched, {task.failed} failed.")
            ...
            # Build flash message with state-by-state results
            states_text = len(state_results)
            result_summary = ", ".join(scraper_results[:5])
            if len(scraper_results) > 5:
                result_summary += f", ... and {len(scraper_results) - 5} more"
            
            flash(f'Fetched {saved} new leads from {states_text} states! ({result_summary}). Enrichment running in background. Refresh to see updates.', 'success')
        elif saved > 0:
            # Build flash message with state-by-state results
            states_text = len(state_results)
            result_summary = ", ".join(scraper_results[:5])
            ...
            flash(f'Fetched {saved} new leads from {states_text} states! ({result_summary}). {duplicates} duplicates found and removed.', 'success')
            
            # Auto-export new leads (raw)
            try:
                auto_export_to_sheet(all_records)
            except Exception as e:
                print(f"Auto-export error: {e}")
        elif all_records:
    ...
    # Build fetch report for UI (prevents redirecting to missing endpoint).
    try:
        def _present(val):
            if val is None:
                return False
            s = str(val).strip().lower()
            return s not in ('', 'none', 'nan', 'not found', '-')

        report_df = db.get_leads_by_ids(new_lead_ids) if 'new_lead_ids' in locals() and new_lead_ids else pd.DataFrame()
        report_records = report_df.to_dict('records') if not report_df.empty else []

        saved_category = sum(1 for r in report_records if _present(r.get('industry_category')))
        saved_owner = sum(1 for r in report_records if _present(r.get('owner_name')))
        saved_phone = sum(1 for r in report_records if _present(r.get('phone')) or _present(r.get('phone_1')) or _present(r.get('phone_2')))
        saved_email = sum(1 for r in report_records if _present(r.get('email')) or _present(r.get('email_1')) or _present(r.get('email_2')))

        state_summary = []
        for code, payload in sorted((state_results or {}).items(), key=lambda x: x[0]):
            state_summary.append({
                'code': code,
                'name': US_STATES.get(code, code),
                'scraped': int((payload or {}).get('count', 0))
            })
        ...
        print(f"[FetchReport] Failed to build report: {report_e}")

    return redirect(url_for('fetch_report'))
...
def api_export_sheets_stream():
    ...
    try:
        ...
        if spreadsheet_id:
            ...
            title = f"Appended Leads - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
